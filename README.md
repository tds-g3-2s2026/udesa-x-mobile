...
autenticada se puede recorrer contra la API real. Para eso está el mock de los cuatro
endpoints de `/auth`, que no tiene dependencias y corre con la biblioteca estándar de
Python:
...
app/                      Rutas de Expo Router
  _layout.tsx             Restaura la sesión y monta el grupo permitido
  (auth)/                 Login y verificación de email
  (auth)/register/        Wizard de registro: un paso por ruta
  (app)/                  Experiencia autenticada
